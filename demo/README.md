...
# override target:
FALDA_URL=http://<tailnet-aggregator>:8078 FALDA_TENANT=default FALDA_TENANT_B=kukla bash demo/falda_demo.sh
```
...
2. Live-plant a fresh fact:
   ...
   r=urllib.request.urlopen(urllib.request.Request("http://<tailnet-aggregator>:8078/atoms/upsert",d,{"Content-Type":"application/json"}))
   print(r.read().decode())
   PY
   ```
