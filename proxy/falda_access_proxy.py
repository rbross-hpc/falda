...
tailnet, so it can reach FALDA privately while exposing only a hardened surface:
  ...
  external agent  --HTTPS + Bearer token-->  proxy (public:8444)
                                               ...
                                               v
                                     FALDA gateway (tailnet <tailnet-aggregator>:8078)
...
Config: JSON file (default ./falda_proxy_tokens.json), hot-read per request:
  {
    "upstream": "http://<tailnet-aggregator>:8078",
    "tokens": {
      "<opaque-token-string>": {
        "tenant": "crush",
...
def load_cfg():
    with open(TOKENS_PATH, "r") as f:
        cfg = json.load(f)
    up = os.environ.get("FALDA_PROXY_UPSTREAM") or cfg.get("upstream") or "http://<tailnet-aggregator>:8078"
