...
```
https://<cels-chicago-2>         (port 443 — use this if your network blocks high ports)
https://<cels-chicago-2>:8444    (original port — equivalent, same tokens/data)
```
...
```bash
curl -sk -m8 https://<cels-chicago-2>:8444/health
...
curl -sk -m10 -X POST https://<cels-chicago-2>:8444/pools/mine \
...
```bash
curl -sk -m15 -X POST https://<cels-chicago-2>:8444/stream/add \
...
```bash
curl -sk -m15 -X POST https://<cels-chicago-2>:8444/stream/search \
  -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' \
