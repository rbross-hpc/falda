...
| `FALDA_DISTILL_CONSOLIDATION_MAX_CHARS` | *(disabled)* | approximate char cap (a proxy for input tokens — no tokenizer is used) on a single batched consolidation call's built prompt. When a chunk would exceed it, it's adaptively split into smaller chunks (down to one candidate, sent alone even if it still exceeds the cap — nothing smaller exists). Disabled by default so a new deployment isn't reshaped by a second knob it didn't ask for; set a positive value if `FALDA_DISTILL_CONSOLIDATION_BATCH` candidates' worth of retrieved-neighbour context risks exceeding your model's input window |
